...
const [isDarkMode, setIsDarkMode] = useState(() => localStorage.getItem('taskly_theme') === 'dark');
...
const [selectedProjectFilter, setSelectedProjectFilter] = useState('הכל');
...
id: audit_${Date.now()},
text: 🔄 הסטטוס שונה ل-"${newStatus}" על ידי ${currentUser},
author: 'מערכת',
time: formattedDate
};
        ...
        <div style={{ backgroundColor: theme.cardBg, color: theme.textMain, borderRadius: '24px', padding: '28px', width: '100%', maxWidth: '540px', maxHeight: '85vh', display: 'flex', flexDirection: 'column', border: `1px solid ${theme.border}` }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <button onClick={() => {
              ...
              if (relevant.length === 0) { alert("לא נבחרו משימות לשיתוף."); return; }
            ...
            }} style={{ flex: 1, padding: '12px', backgroundColor: '#25d366', color: '#fff', border: 'none', borderRadius: '10px', fontWeight: '800', cursor: 'pointer' }}>שלח בוואטסאפ</button>
    ...
    <div style={{ backgroundColor: theme.cardBg, borderRadius: '16px', padding: '16px 20px', border: `1px solid ${theme.border}`, marginBottom: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span style={{ fontSize: '14px', fontWeight: '800' }}>📁 פרויקט:</span>
          <select value={selectedProjectFilter} onChange={(e) => setSelectedProjectFilter(e.target.value)} style={{ padding: '10px 16px', borderRadius: '12px', border: `1px solid ${theme.border}`, backgroundColor: theme.inputBg, color: theme.inputText, fontSize: '14px', fontWeight: '700' }}>

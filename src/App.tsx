...
export default function App() {
  ...
  // מסך הכניסה המקורי והמעוצב בדיוק לפי בקשתך
  if (!currentUser) {
    return (
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: theme.bg, color: theme.textMain, padding: '24px 16px', direction: 'rtl', fontFamily: FONT_FAMILY, transition: 'background-color 0.2s' }}>
      ...
      <div style={{ maxWidth: '1440px', margin: '0 auto' }}>
        ...
        {/* מודאל עריכת משימה (כולל דחיות ותאריך השלמה) */}
        {editingTask && (
          <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(3px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '16px' }}>
            <div style={{ backgroundColor: theme.cardBg, color: theme.textMain, borderRadius: '24px', padding: '28px', width: '100%', maxWidth: '520px', border: `1px solid ${theme.border}` }}>
              ...
              <form onSubmit={handleSaveEditedTask} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  <div style={{ flex: 1 }}>
                    <label style={{ display: 'block', fontSize: '13px', fontWeight: '700', marginBottom: '4px' }}>מספר דחיות</label>
                    <input type="number" value={editingTask.delays} onChange={(e) => setEditingTask({ ...editingTask, delays: parseInt(e.target.value) || 0 })} style={{ width: '100%', padding: '11px 12px', borderRadius: '10px', border: `1px solid ${theme.border}`, backgroundColor: theme.inputBg, color: theme.inputText }} />
                  </div>
                  <div style={{ flex: 1 }}>
                    <label style={{ display: 'block', fontSize: '13px', fontWeight: '700', marginBottom: '4px' }}>תאריך השלמה</label>
                    <input type="date" value={editingTask.completedDate || ''} onChange={(e) => setEditingTask({ ...editingTask, completedDate: e.target.value })} style={{ width: '100%', padding: '11px 12px', borderRadius: '10px', border: `1px solid ${theme.border}`, backgroundColor: theme.inputBg, color: theme.inputText }} />
                  </div>
                </div>
        ...
        {viewMode === 'dashboard' ? (
          ...
          /* תצוגת טבלה או כרטיסיות (מציגה רק פרויקטים עם משימות תואמות בעת סינון) */
          allProjectNames
            .filter((p) => {
